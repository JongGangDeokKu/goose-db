...
class GooseDB {
    ...
    async createDB(dbName) {
        ...
        const columns = ["TABLE_NAME", "COLUMN_NAME", "DATA_TYPE", "NULLABLE", "DATA_DEFAULT", "COLUMN_ID", "COMMENTS", "PK", "FK"]
        const colRes = await this.addColumn("0_SCHEMA_TABLE", [null, ...columns]);
            ...
            async function authGoogleSheet(){
                ...
                try{
                    ...
                    for(var i = 0; i<result.length; i++){
                        for (var j = 0; j < up_columns.length; j++){
                            rows[Number(result[i][0])-1][up_columns[j]] = up_values[j]
                            arrs.push(save_rows(rows[Number(result[i][0])-1])) 
                        }
                    }
                    await Promise.all(arrs)
                }catch(err){
